...
clear all;
mat1 = 1: 12;
mat1 = reshape(mat1, [3, 4]);
% size()
rowCount = size(mat1, 1);
colCount = size(mat1, 2);
% numel()——矩阵元素数
elemCount = numel(mat1);
...
% reshape()——重构矩阵（先按列逐个获取元素，然后按列排列成指定的尺寸矩阵）
...
% 矩阵的条件表达式返回一个逻辑矩阵，其中满足条件的元素为1，不满足的为0；
M1 =  [0, 8.1, 7.3; -6.2, 0, 4.4; -3.1, 2.5, 1.1];
IM1 = (M1 < 0);     
M2 = M1.*IM1;               % 与逻辑矩阵点乘，可以筛选元素，返回筛选元素后的矩阵；
M3 = M1(IM1);               % 逻辑矩阵作为operator()的参数，可以提取矩阵中的元素；

% 索引矩阵作为operator()的参数——如V1(V2), 提取V1中对应索引的元素；
V1 = -(1:10);
V2 = [1, 1, 8, 10, 6, 5, 4, 3, 7, 8]';
V3 = V1(V2);
...
clc;
clear all;
% C = ismember(A, B)返回一个尺寸和A相同的逻辑矩阵，aij若存在于B中，则cij为true，否则为false;
m1 = [1,2,3; 4,5,6];
v1 = [1,1,1,6,7,8,9,10,12, 13]';
result = ismember(v1, m1);
