function W = biharmonic_bounded(V,F,b,bc,type,pou,low,up)
  ...
  % Build discrete laplacian and mass matrices used by all handles' solves 建立离散的拉普拉斯矩阵和质量矩阵
  if(size(F,2)==4)
  ...
  if(pou)
    ...
    if(strcmp(type,'quad'))
      ...
      fprintf( [ ...
        '  minimize:     x''LM\\Lx\n' ...
        'subject to: %g <= x <= %g, 鈭慱i xi = 1\n'], ...
        low,up);
      tic;
      W = quadprog(Q,zeros(n*m,1),[],[],[PA;BCA],[Pb;BCb],lx,ux,[],param);% quadprog具有线性约束的二次目标函数的求解器
      toc
      W = reshape(W,n,m);
    else
      error( [ ...
    ...
    for i = 1:m
      ...
      elseif(strcmp(type,'least-squares'))
        ...
        fprintf([ ...
          ...
          '  and          %g <= x <= %g\n'], ...
          low,up);
        x = quadprog(Q,zeros(2*n,1),[],[],B,c,lx,ux,[],param);%具有线性约束的二次目标函数的求解器
      elseif(strcmp(type,'conic'))
        prob.bux(b) = bc(:,i);
        prob.blx(b) = bc(:,i);
