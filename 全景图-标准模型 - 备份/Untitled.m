...
%     fdi = textread('FDI__.txt');
    ...
    if isempty(n)
        ...
        p_bingren =[c(1),y(1),c(3)];n_bingren = axi(2,:);
        v_bingren = bingren.vertex;f_bingren = bingren.face;
...
%         plot_edges(v_bingren,lone_edges_list,'r','LineWidth',2);
        ...
        for i = 1:length(edge_p_b_local)
            index = find(lone_edges_list == edge_p_b_local_list(i));
            Edge_b(index) = edge_p_b_local(i);
        end
        ...
        for j = 1:length(point_bingren)
            index = find( ff_b == index_bingren(j));
            f_b0(sub2ind(size(f_b0), index)) = j;
        end
...
%         point11 = point1(randperm(length(point1),length(point2)),:);
        ...
        for j = 1:length(tooth)
            index = find( ff_t == index_tooth(j));
            f_t0(sub2ind(size(f_t0), index)) = j+length(point_bingren);
        end
        ...
        temp1 = v_end(edg_b(1,1),:) - mean(edge_p_b);
        s_b = sign(dot(temp1, cross(tri11,tri12)));
        
        temp2 = v_end(Edge_t(r,1),:) - mean(edge_p_t);
        s_t = sign(dot(temp2,cross(tri21,tri22)));
...
%         pats = [];
    ...
    if isempty(n)
       ...
       return;
        ...
        f_end = [f_b0;f_t0];
        v_end = [point_bingren;tooth];
        
        
        writeOBJ('补三角片前的合并网格.obj', v_end, f_end);
        OBJwriteVertices('切割牙根顶点.obj', tooth); 
        ...
        dt = [];
        middle = zeros(length(edg_t), 3);
        for j = 1:length(edg_t)
            p1 = v_end(edg_t(j,1),:);
            p2 = v_end(edg_t(j,2),:);
            middle(j, :) = (p1+p2)/2;
            [~,ro]= mindis(v_end(edg_b(:,1),:),middle(j, :), 1);
            roww(j) = ro;
            dt = [dt;[edg_t(j,:),edg_b(ro,1)]];      
        end
