...
%% 在于光线垂直面上的牙根变形
%单颗牙齿，找到三维模型上的参考平面，将全景图上相应的牙齿投射到参考平面上，作为三维牙齿变形的参考
%在作此之前要控制好根尖点的位置
for n = 28:33
    j =1;
    face = [];
...
%         n_loc = max(n_clo);
    ...
    n = fitNormal(Edge_points,1);
%     
% %     ppoint = sortrows(Edge_points,3);
%     midpoint = Edge_points(find(Edge_points(:,3)==min(Edge_points(:,3))),:);
%     ppoint1 = Edge_points(find(Edge_points(:,2)>=midpoint(2)),:);ppoint2 = Edge_points(find(Edge_points(:,2)<midpoint(2)),:);
%     ppoint = [sortrows(ppoint1,3,'descend');sortrows(ppoint2,3)];
%     points = mat2cell(ppoint(:,1:3),ones(1,length(ppoint)));
...
for M = 1:NN
    dir = [FX(M),FY(M),0];
    j = 1;
    for z = 30:-0.3:-30%min(vertices(:,3)):0.1:max(vertices(:,3))
        orig = [pt(M,1)+distance*FX(M)/(FX(M).^2+FY(M).^2).^0.5,pt(M,2)+distance*FY(M)/(FX(M).^2+FY(M).^2).^0.5,z];
%         plot3(orig(1),orig(2),orig(3),'*r')
        [intersect, t, u, v] = TriangleRayIntersection(orig, dir, vert1, vert2, vert3); 
        if sum(intersect)>=2
            
            d = t(find(intersect~=0));
            if mod(sum(intersect),2)==0
                a = d(1:2:end-1);b = d(2:2:end);
                Spix(j,M) = abs(sum(b-a));
            end
        else
            Spix(j,M)=0;
        end
...
%     i = 1; 
%     % dir = [1,0,0];
%     % [intersect, t, u, v] = TriangleRayIntersection(orig, dir, vert1, vert1, vert1); 
% 
%     for theta = -pi/2:0.01:pi/2
%         c = 75;
%         dir = [cos(theta)*cos(beta),cos(theta)*sin(beta),sin(theta)];
% %         if beta <= pi/2
% %             dir = [sqrt(cos(theta)*cos(theta)-sin(beta)*sin(beta)),sin(beta),sin(theta)];
% %         else
% %             dir = [-sqrt(cos(theta)*cos(theta)-sin(beta)*sin(beta)),sin(beta),sin(theta)];
% %         end
%         [intersect, t, u, v] = TriangleRayIntersection(orig, dir, vert1, vert2, vert3); 
