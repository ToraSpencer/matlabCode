...
V = mesh_source.vertex(:,1:3);
l = length(mesh_source.vertex);
F = [l+mesh_source.face(:,3)+1,l+mesh_source.face(:,2)+1,l+mesh_source.face(:,1)+1];
% upsample_mesh = false;
% 
% if(~isempty(regexp(mesh_source,'\.(off|obj)$')))
%   % load a mesh from an OBJ
%   [V,F] = load_mesh(mesh_source);
%   %顶点存储V中,在2维空间中,V为 #V * 2矩阵
%   %面的索引存储在F中,在2维空间中,F为 #F * 3矩阵
...
N = size(find(V(:,3)>=-11&V(:,3)<-10.5),1);
C = [V(find(V(:,3)>=-11&V(:,3)<-10.5),:);V(find(V(:,3)>=max(V(:,3)-0.1)),:);V(find(V(:,3)<=min(V(:,3))+0.2),:)];
...
if(exist('mosekopt','file'))
  ...
  W = biharmonic_bounded(V,F,b,bc,'least-squares',false);
end
% Normalize weights
...
simpledeformautom(V,F,C,W,N,'InterpMode','LBS');
% simple_deform(V,F,C,W,'InterpMode','DQLBS');
