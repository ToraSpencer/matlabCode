function varargout = simple_deform(varargin)
  ...
  g_Deform(gid).tsh = trisurf(F,V(:,1),V(:,2),V(:,3), ...
    'FaceColor','interp');
  % 2D view
  view(3);
  axis equal
  ...
  win_min = min([C(:,1:3); V(:,1:3)]);
  win_max = max([C(:,1:3); V(:,1:3)]);
  ...
  function oncontrolsdown(src,ev)
    ...
    down_pos=[down_pos(1,1,1) down_pos(1,2,1) down_pos(1,3,1)];
    ...
    [minD,ci] =  ...
      min(sum((g_Deform(gid).new_C(:,1:3) - ...
      repmat(down_pos,size(g_Deform(gid).new_C,1),1)).^2,3));
    % keep track of mesh vertices at mouse down
  ...
  function oncontrolsdrag(src,ev)
    ...
    drag_pos=[drag_pos(1,1,1) drag_pos(1,2,1) drag_pos(1,3,1)];
  ...
  function update_positions()
    ...
    if(nb > 0)
      set(B_plot_outer,{'XData'}, num2cell([ ...
        ...
        g_Deform(gid).new_C(BE(:,2),1) ...
        g_Deform(gid).new_C(BE(:,3),1)],3));
      set(B_plot_outer,{'YData'}, num2cell([ ...
        g_Deform(gid).new_C(BE(:,1),2) ...
        g_Deform(gid).new_C(BE(:,2),2) ...
        g_Deform(gid).new_C(BE(:,3),2)],3));
    set(B_plot_outer,{'ZData'}, num2cell([ ...
        g_Deform(gid).new_C(BE(:,1),3) ...
        g_Deform(gid).new_C(BE(:,2),3) ...
        g_Deform(gid).new_C(BE(:,3),3)],3));
      ...
      set(B_plot_inner,{'YData'}, num2cell([ ...
        ...
        g_Deform(gid).new_C(BE(:,2),2) ...
        g_Deform(gid).new_C(BE(:,3),2)],3));
    set(B_plot_inner,{'ZData'}, num2cell([ ...
        g_Deform(gid).new_C(BE(:,1),3) ...
        g_Deform(gid).new_C(BE(:,2),3) ...
        g_Deform(gid).new_C(BE(:,3),3)],3));
    end
    ...
    if(size(PE,1)>0)
      set(PE_plot,{'XData'}, num2cell([ ...
        ...
        g_Deform(gid).new_C(P(PE(:,2)),1) ...
        g_Deform(gid).new_C(P(PE(:,3)),1)],3));
      set(PE_plot,{'YData'}, num2cell([ ...
        g_Deform(gid).new_C(P(PE(:,1)),2) ...
        g_Deform(gid).new_C(P(PE(:,2)),2) ...
        g_Deform(gid).new_C(P(PE(:,3)),2)],3));
      set(PE_plot,{'ZData'}, num2cell([ ...
        g_Deform(gid).new_C(P(PE(:,1)),3) ...
        g_Deform(gid).new_C(P(PE(:,2)),3) ...
        g_Deform(gid).new_C(P(PE(:,3)),3)],3));
    end
    % update cage edge plots
    if(size(CE,1)>0)
      set(CE_plot_outer,{'XData'}, num2cell([ ...
        g_Deform(gid).new_C(P(CE(:,1)),1) ...
        g_Deform(gid).new_C(P(CE(:,2)),1) ...
        g_Deform(gid).new_C(P(CE(:,3)),1)],3));
      set(CE_plot_outer,{'YData'}, num2cell([ ...
        g_Deform(gid).new_C(P(CE(:,1)),2) ...
        g_Deform(gid).new_C(P(CE(:,2)),2) ...
        g_Deform(gid).new_C(P(CE(:,3)),2)],3));
     set(CE_plot_outer,{'ZData'}, num2cell([ ...
        g_Deform(gid).new_C(P(CE(:,1)),3) ...
        g_Deform(gid).new_C(P(CE(:,2)),3) ...
        g_Deform(gid).new_C(P(CE(:,3)),3)],3));
    ...
    % update mesh positions
    set(g_Deform(gid).tsh,'Vertices',new_V(:,1:3));
  end
