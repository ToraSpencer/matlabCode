...
%         centcrownintooth = p_crown_biaozhun+point_tooth-point_crown;%切后标准根的边缘中心
        centcrownintooth = mean(vertex_tooth_biaozhun(index_tooth_biaozhun,:));
...
%         end
        ...
        tri11 = v_end(edg_b(4,1),:) - v_end(edg_b(1,1),:);tri12 = v_end(Edge_t(r,1),:) - v_end(edg_b(4,1),:);
        tri21 = v_end(Edge_t(rr,3-rm),:) - v_end(Edge_t(r,1),:);tri22 = v_end(edg_b(1,1),:) - v_end(Edge_t(rr,3-rm),:);
        s_b = sign(dot(v_end(edg_b(1,1),:) -c,cross(tri11,tri12)));
        s_t = sign(dot(v_end(Edge_t(r,1),:) - c,cross(tri21,tri22)));
        if s_b ~= s_t
...
%         hold on
        for j = 1:length(edg_t)
            ...
            if j >=2 && ro < roww(j-1) && roww(j-1) - ro<length(edg_t)/4
                ro = roww(j-1);
            end
            roww(j) = ro;
%             rr = edg_t(j,:);
...
%            roww = [roww roww(1:s(1)-1)];
        ...
        ff = [f_end;dt];
        figure()
        plot_edges(v_end,edg_b,'r','LineWidth',2);
        hold on
        plot_edges(v_end,edg_t,'r','LineWidth',2);
        trisurf(dt,v_end(:,1),v_end(:,2),v_end(:,3),'facecolor','c','edgecolor','b')
